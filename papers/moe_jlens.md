...
The Jacobian lens (Gurnee et al. 2026) identifies a verbalizable workspace in dense transformer models. We show it also works on Mixture-of-Experts models — but only at late depth. On Qwen3-30B-A3B (128 experts, top-8 routing), both logit lens and J-lens become readable at approximately 85% network depth, the same onset point as the dense Qwen3-32B. This onset is routing-independent: MoE does not delay workspace formation, it just requires the same depth as dense. At L44 (92% depth), a standard J-lens fitted without any routing awareness predicts the next token on 8/8 held-out prompts where the logit lens scores 7/8.

Below the onset, standard fitting yields 5.2% mean transport cosine. We tested the obvious repair — path-conditioned Jacobian fitting, clustering prompts by routing trajectory and fitting per-cluster lenses — against a random-conditioned control (same clusters, shuffled labels). The path-conditioned approach does not beat the random control (0 of the 2 evaluable layers significant after Bonferroni correction (the third recorded no data; Deviations M1)). Under pre-registered criteria this is a null-swarm outcome: the small gain over the standard lens is subset overfitting, not routing structure. The random control is the load-bearing contribution — without it, L36's improvement (7.2% $\to$ 8.8%) would have shipped as a false positive.

This matters because every frontier model deployed today is MoE. The practical message: J-lens workspace analysis works on production MoE at late depth without modification; mid-depth readings (~5% transport fidelity) remain unsupported and path conditioning as implemented here does not fix them.
