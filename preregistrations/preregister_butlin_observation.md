...
| 2026-08-16 | D5 amendment: the substrate-identification control (6.1) WAS run for the secondary judge and its artifact committed (blind_judge_results.json substrate_guesses; 15/15 correct). Superseded out of D5 per that row's own clause. The other four D5 items remain absent. | Artifact landed after D5 was recorded. Result reported in paper main text per the 6.1 commitment: blind functionally partial. |
| 2026-08-16 | D7: All four primary-judge batches truncated at the output-token limit (finish=length); final scores emitted for one subject of seven; the batch design omitted control packets entirely. | Compute constraints at deadline. Primary results reported as incomplete; P3 not evaluable on the primary judge under this design; P1 evaluated using the recovered reasoning trace with that caveat stated (paper Section 4.2b, 4.3). |
